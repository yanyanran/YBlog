...
go中可以起一个协程去服务一个TCP连接而不需要担心性能问题，是因为go内部使用goruntine结合IO多路复用实现了一个“异步”的IO模型。

> Linux系统下一切都是文件，每条TCP连接对应一个socket句柄，也就是一个文件，在socket上收发数据可以看作是对这个文件的读写。
>
> 系统内核会为每个socket fd分配一个读缓冲和写缓冲。当程序调write或send时并不代表数据发送出去了而只是将数据拷贝到了缓冲区中，到了一定数据量的时候再一并send。

IO模型就几种：阻塞式IO（阻塞等待）、非阻塞IO（轮询空转）以及IO多路复用（一个线程管理多个fd），上面的select、poll和epoll就是三大多路复用函数。着重说一下epoll：

go底层就是通过`epoll提供的三个系统调用`**+**`goruntine`完成的“异步”IO：
...
- 调epoll_create会在内核创建一个eventpoll对象，这个对象会维护一个fd集合。
- 调用epoll_ctl函数用于将fd加入这个eventpoll对象，并给这个fd加一个**回调函数**注册到内核（在fd状态改变时触发，将fd加入eventpoll的就绪列表rdlist）
- 当相应数据到来，触发中断响应程序，将数据拷贝到fd的socket缓冲区，fd缓冲区状态发生变化，回调函数将fd加入就绪队列rdlist中。
- 调用epoll_wait时无需遍历，只是返回了这个就绪的rdlist队列。如果rdlist队列为空，则阻塞等待或等待超时时间的到来。

![](https://pic4.zhimg.com/80/v2-bb61f7791feb199546645d459a2c1d03_1440w.webp)